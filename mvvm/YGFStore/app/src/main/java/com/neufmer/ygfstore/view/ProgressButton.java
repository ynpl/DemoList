...
public class ProgressButton extends TextView {
    ...
    public static class SimpleOnProgressClickListener implements OnProgressClickListener {
        @Override
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mBackgroundColor = a.getColor(R.styleable.ProgressButton_backgroud_color, Color.parseColor("#6699ff"));
        mBackgroundSecondColor = a.getColor(R.styleable.ProgressButton_backgroud_second_color, Color.LTGRAY);
        mButtonRadius = a.getFloat(R.styleable.ProgressButton_radius, getMeasuredHeight() / 2);
        mTextColor = a.getColor(R.styleable.ProgressButton_text_color, mBackgroundColor);
        mTextCoverColor = a.getColor(R.styleable.ProgressButton_text_covercolor, Color.WHITE);
        backgroud_strokeWidth = a.getDimension(R.styleable.ProgressButton_backgroud_strokeWidth, 3F);
        mNormalText = a.getString(R.styleable.ProgressButton_text_normal);
        mDowningText = a.getString(R.styleable.ProgressButton_text_downing);
        mFinishText = a.getString(R.styleable.ProgressButton_text_finish);
        mPauseText = a.getString(R.styleable.ProgressButton_text_pause);
        mAnimationDuration = a.getInt(R.styleable.ProgressButton_animation_duration, 500);
        a.recycle();
    }
    ...
    private void init() {
        ...
        setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (mOnProgressClickListener == null) {
                    return;
                }
                if (getState() == NORMAL) {
                    mOnProgressClickListener.clickProgress();
                    setState(ProgressING);
                    setProgressText(0);
                } else if (getState() == ProgressING) {
                    if (mEnablePause) {
                        mOnProgressClickListener.clickPause();
                        setState(PAUSE);
                    }
                } else if (getState() == PAUSE) {
                    mOnProgressClickListener.clickResume();
                    setState(ProgressING);
                    setProgressText((int) mProgress);
                } else if (getState() == FINISH) {
                    mOnProgressClickListener.clickFinish();
                }
            }
        });
    }
    ...
    private void setProgressText(int progress) {
        if (getState() == ProgressING) {
//            setCurrentText(mDowningText + progress + "%");
            setCurrentText(mDowningText);
        }
    }
    ...
    public OnProgressClickListener getOnProgressClickListener() {
        return mOnProgressClickListener;
    }

    public void setOnProgressClickListener(OnProgressClickListener onProgressClickListener) {
        mOnProgressClickListener = onProgressClickListener;
    }
